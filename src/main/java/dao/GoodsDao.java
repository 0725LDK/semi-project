...
public class GoodsDao {
	...
	public ArrayList<HashMap<String, Object>> selectGoodsList(Connection conn) throws Exception {
		...
		String sql = "SELECT g.goods_code goodsCode, g.goods_name goodsName, g.goods_category goodsCategory,  g.goods_content goodsContent, img.filename filename"
	...
	public HashMap<String, Object> selectgoodsOne(Connection conn, int goodsCode) throws Exception {
		...
		String sql = "SELECT g.goods_code goodsCode, g.goods_name goodsName, g.goods_price goodsPrice, g.goods_category goodsCategory, g.goods_content goodsContent, g.soldout soldout, g.emp_id empId, g.hit hit, g.createdate createdate, img.filename filename, img.origin_name originName"
				+ " FROM goods g INNER JOIN goods_img img"
				+ " ON g.goods_code = img.goods_code"
				+ " WHERE g.goods_code = ?";
		PreparedStatement stmt = conn.prepareStatement(sql);
	...
	public int updateGoods(Connection conn, Goods goods) throws Exception {
		String sql = "UPDATE goods SET goods_name = ?, goods_price = ?, goods_category = ?, goods_content = ?, soldout = ?, emp_id = ?, hit = ? WHERE goods_code = ?";
		PreparedStatement stmt = conn.prepareStatement(sql);
		stmt.setString(1, goods.getGoodsName());
		stmt.setInt(2, goods.getGoodsPrice());
		stmt.setString(3, goods.getGoodsCategory());
