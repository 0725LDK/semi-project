...
-- 테이블 데이터 shop.goods_img:~0 rows (대략적) 내보내기
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='새로운 아이디 생성시 outid + emp + customer 존재하지 않는 아이디 생성가능';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='포인트 이력테이블';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='pw변경이력이 고객댕 최근 3개까지만';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='orders테이블에서 order_state값이 구매확정이 되면 고객이 review 등록 가능';
